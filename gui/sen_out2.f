...
C             SQ constraint
     ...
     &            excel_sep, bus(nb), excel_sep, base(nb), excel_sep, 
     &            arcnam(jt), excel_sep, datk, excel_sep, excel_sep, 
     &            datk*xatot, excel_sep, pct1, excel_sep, xatot,
     &            excel_sep, arcnam(jarzn(nb))
10180           format (t2, a2, a, a3, a, 'A', a, a8, f6.1, 
     &            1x, '(', a, ')', t51, a, f8.2, t62, a, f8.1, 
     &            a, f8.1, t81, a, f7.1, ' MW', t110, a, a)
              else
...
C             SV constraint
     ...
     &            arcnam(jarzn(nb))
10200           format (t2, a2, a, a3, a, 'A', a, a8, 
     &            f6.1, 1x, '(', a, ')', t51, a, f8.2, t62, a,
     &            f8.1, a, f8.1, t81, a, f7.1, ' MW', t110, 
     &            a, a)
              else
...
C           PQ constraint
     ...
     &          excel_sep, excel_sep, type, excel_sep, bus(nb), 
     &          excel_sep, base(nb), excel_sep, dptk, excel_sep, 
     &          dptk*pnetx, excel_sep, pct1, excel_sep, pnetx, 
     &          excel_sep, arcnam(jarzn(nb))
10210         format (t2, a2, a, a3, a, 'B', a, a, a8, f6.1,
     &          t51, a, f8.2, t62, a, f8.1, a, f8.1, t81, 
     &          a, f7.1, ' MW', t110, a, a)
            else
...
10222         format (t2, a2, 3x, a3, 3x, 'B', a, 1x, a8, f6.1, t51, 
     ...
     &          excel_sep, bus(k), excel_sep, base(k), excel_sep, 
     &          bus(m), excel_sep, base(m), excel_sep, id, excel_sep, 
     &          sect, excel_sep, dptk, excel_sep, dptk*pin, excel_sep, 
     &          pct1, excel_sep, pin, excel_sep, angle, excel_sep, 
     &          arcnam(jarzn(k))
10330         format (t2, a2, a, a3, a, 'RP', a, a8, f6.1, 
     &          1x, a8, f6.1, 1x, a1, 1x, i1, t51, a, f8.2, t62, 
     &          f8.1, a, f8.1, t81, a, f7.1, ' MW @ ', 
...
10332         format (t2, a2, 3x, a3, 3x, 'RP', a, a8, f6.1, 1x, a8,
     &          f6.1, 1x, a1, 1x, i1, t51, a, f8.2, t62, f8.1, f8.1,
     &          t81, a, f7.1, ' MW @ ', f7.1, ' degrees', t110, 
     &          a, a)
            endif
            ...
            pct1 = dptk
            if (excel_flag) then
              write (outbuf, 10340) zone(k), excel_sep, cbown, i
     &          excel_sep, bus(k), excel_sep, base(k), excel_sep, 
     &          bus(m), excel_sep, base(m), excel_sep, id, excel_sep, 
     &          sect, excel_sep, dptk, excel_sep, dptk*angle, 
     &          excel_sep, pct1, excel_sep, angle, excel_sep, 
     &          arcnam(jarzn(k))
10340         format (t2, a2, a, a3, a, 'TP', a, a8, f6.1, 
     &          1x, a8, f6.1, 1x, a1, 1x, i1, t51, a, f8.2, t62, 
     &          a, f8.1, a, f8.1, t81, a, f7.1, ' Degrees',
     &          t110, a, a)
            else
...
10480   format (t2, 'Total:', t9, f8.1, f7.1)
...
10488   format (t2, 'Ownership Analysis for branch: ', a8, f7.1, 1x, 
     &    a8, f7.1, 1x, a1, ' Flow = ', f7.1, ' MW, ', f7.1, ' MVAR)')
        ...
        write (scrfil, 10490)
10490   format (t2, 'Ownership   /----- Owner Net ------/     /----- Gen
     &eration -----/ /-------- Load --------/ /------D-C System------/')
...
10496   format (t2, '              (MW)           (MW)         (MW)     
     ...
     &       syst(4,jo)-syst(8,jo)
10498      format (t5, a3, t11, f9.1, f7.1, f9.1, 4x, 
     &       3(f9.1, f7.1, f9.1))
        enddo
...
10502   format (t2, 'Total', t11, f9.1, f7.1, f9.1, 4x,
     &    3(f9.1, f7.1, f9.1))
      endif
