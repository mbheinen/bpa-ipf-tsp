/******************************************************************************\
UTILITY:    QVcurve_Gen - _Pro with Qgen output at each point on Q-V curve.
...
#define  DOT_LINE         "..................................................."
#define  LOG_SPEC         "qvcurve_gen.log"
#define  OUT_NAME         "qvcurve_gen"
...
#define  QUERY_GEND  (int) (1<<20) /* prompt for /GEN_DROP card data */
...
#define  READ_GDR    (int)     17  /* found /GEN_DROP     in CFLOW data file */
...
#define  FAIL_CURV   (int) (1<< 6)
#define  FAIL_VATQ   (int) (1<< 7)
#define  FAIL_QATV   (int) (1<< 8)
#define  FAIL_USAN   (int) (1<< 9)
...
typedef struct {
  ...
  float  Qres;
  float  Vgen;
  float  Vbus; /* voltage at perturbed bus */
} ff_soln;
...
typedef struct traceRecord {
  ...
  char   change_bus_types[CF_IPCSIZE];
  char   agc[CF_IPCSIZE];
  char   gen_drop[CF_IPCSIZE];
  float  VXmax;
  float  VXmin;
  float  dVX;
...
void   ff_printCaseReport(ff_Report *rpt, Step *step);
void   ff_printQminReport(ff_Report *rpt, Step *step, int stat);
void   ff_printFlowReport(ff_Report *rpt, Step *step, Link *cutpList);
void   ff_printQresReport(ff_Report *rpt, Step *step, int stat, Link *gbusList);
void   ff_printCaseHeader(ff_Report *rpt);  
void   ff_printQminHeader(ff_Report *rpt);  
void   ff_printQresHeader(ff_Report *rpt);
void   ff_printFlowHeader(ff_Report *rpt);
void   ff_printQgenHeader(ff_Report *rpt, Step *step, Link *gbusList);
void   ff_printCasePoints(ff_Report *rpt, Step *step);
void   ff_printQgenPoints(ff_Report *rpt, Link *gbusList, float V);
void   ff_printQminPoints(ff_Report *rpt, Step *step, int stat);
void   ff_printQresPoints(ff_Report *rpt, Step *step, Link *gbusList);
void   ff_printFlowPoints(ff_Report *rpt, Step *step, Link *cutpList);
void   printHdrUnits(FILE *fp, Link *curLink, char *l, char *r);
void   printHdrBuses(FILE *fp, Link *curLink, Step *step, char *l, char *r);
ff_Report *ff_initReport(char *fs, FILE *fp, int ln, int pg, char *ts);
...
void   de_qreserve(int *stat, Link *genList, pf_rec busID);
...
int main(int argc, char *argv[])
{
  ...
  cleanUpFiles(&trace);
  time(&CF_time1);
  cf_logUse("QVcurve_Gen", trace.IPFversion, trace.userID);
  return 0;
}
/* end of main() */
void  initializeTrace(Trace *trace)
{
  ...
  trace->query            = ( QUERY_BASE | QUERY_BRCH | QUERY_QBUS |
                              QUERY_USRA | QUERY_NAME | QUERY_SETU |
  ...
  trace->trc = ff_initReport("", NULL, 0, 1, trace->timeStamp);
  trace->rpt = ff_initReport("", NULL, 0, 1, trace->timeStamp);
  trace->gen = ff_initReport("", NULL, 0, 1, trace->timeStamp);
  trace->cut = ff_initReport("", NULL, 0, 1, trace->timeStamp);
  trace->curvNumber       = 0;
...
void instructions(Trace *trace)
{
  printf("\n                      Welcome to QVcurve_Gen - updated 04-09-97");
  printf("\n");
...
void promptUser(Trace *trace)
{
  ...
  if (trace->query & (QUERY_USRA | QUERY_NAME | QUERY_CTRL | QUERY_CURV)) {
    printf("\n\n/REPORT");
  }
  ...
  cf_strsuf(trace->gen->spec, trace->outName, '.', ".gen");
  cf_strsuf(trace->cut->spec, trace->outName, '.', ".cut");
  printf("\n%s\n", DOT_LINE);
  return;
}
void printTrace(Trace *trace)
...
/**************** BEGINNING OF POWERFLOW DISPATCH ENGINE **********************/
  ...
  ff_printMainHeader(trace->cut, trace);
  ...
  for (stepLink = trace->stepList; stepLink != NULL; stepLink = stepLink->next){
    ...
    de_command(&trace->stat, trace->gen_drop, "");
    ...
    ff_printQminReport(trace->rpt, step, trace->stat);
    ff_printQresReport(trace->cut, step, trace->stat, trace->gbusList);
    ff_printFlowReport(trace->cut, step, trace->cutpList);
  }
  ff_printMainFooter(trace->rpt);
  ff_printMainFooter(trace->cut);
...
void  finalRemarks(Trace *trace)
{
  ...
  printf("\nGenerator report written to %s", trace->gen->spec);
  printf("\nCut-plane report written to %s", trace->cut->spec);
  if (trace->trace == TRACE_YES)
...
void de_curve(int *stat, Trace *trace, Step *step, pf_rec *r)
{
  ...
  ff_printCaseReport(trace->gen, step);
  ff_printQgenHeader(trace->gen, step, trace->gbusList);
...
/* the following calculates Q reserve levels */
    ...
    ff_printQgenPoints(trace->gen, trace->gbusList, v);
  }
  ff_printPageFooter(trace->gen, FF_PAGE_LENGTH);
...
/*  cf_bus2rec((cf_Bus *) gbusLink->data, &m); */
    memcpy(&m, gbusLink->data, sizeof(pf_rec));
    pf_rec_bus(&m, "G");                             /* get target QV bus */
    pf_rec_bus(&m, "O");                             /* get target QV bus */
    pf_rec_bus(&busID, "O");                         /* get perturbed bus */
    pf_rec_b2a(id, &m, "I");
    sprintf(name, "%-8.8s", &id[6]);
    sprintf(volt, "%-4.4s", &id[14]);
    ...
    solnData->Vbus = busID.s.ACbus.Vmag;
...
void ff_stream2List(FILE *readMe, Trace *trace, Link **expList)
{
  ...
  while (fgets(str, MAX_IN, readMe)!=NULL) { 
    ...
    if ( strstr(STR, "/GENERATOR"   )!=NULL ) { mode = READ_GEN; trace->query &= ~(QUERY_GBUS); }
...
void ff_printQminReport(ff_Report *rpt, Step *step, int stat)
{
  ...
  ff_printQminPoints(rpt, step, stat);
}
void ff_printQminHeader(ff_Report *rpt)
{
  char *HdQminLd[] = { /* lead */
...
{
  ff_printPageFooter(rpt, 6);
  ff_printCaseHeader(rpt);
  ff_printCasePoints(rpt, step);
  return;
}
void ff_printCaseHeader(ff_Report *rpt)
{
  char *HdLd[] = { /* header lead */
    " ",
    "            |  Primary  |                          |            |",
    "  Basecase  |  Change   |          Outage          |    Bus     |",
    "------------|-----------|--------------------------|------------|",
  };
  fprintf(rpt->file, "%s\n", HdLd[0]);
  fprintf(rpt->file, "%s\n", HdLd[1]);
...
void ff_printCasePoints(ff_Report *rpt, Step *step)
{
  ...
  if (ff_printPageFooter(rpt, 1)) ff_printCaseHeader(rpt);
...
void ff_printQresReport(ff_Report *rpt, Step *step, int stat, Link *gbusList)
{
  ...
  ff_printQminPoints(rpt, step, stat);
  ff_printPageFooter(rpt, 6);
  ff_printQresHeader(rpt);
  ff_printQresPoints(rpt, step, gbusList);
  return;
}
void ff_printQresHeader(ff_Report *rpt)
{
...
void ff_printQresPoints(ff_Report *rpt, Step *step, Link *gbusList)
{
  ...
  while (curGbus!=NULL) {
    ...
    solnLink = solnLink->next;  /* get last quantity */
    solnData = (ff_soln *) solnLink->data;
    cf_aprint(newLine, "%8.2f|", solnData->Qres);
...
void ff_printFlowReport(ff_Report *rpt, Step *step, Link *cutpList)
{
  ...
  ff_printFlowPoints(rpt, step, cutpList);
...
void ff_printQgenHeader(ff_Report *rpt, Step *step, Link *gbusList)
{
  ...
  printHdrUnits(rpt->file, gbusList, HdLd[0], HdRp[0]);
  printHdrUnits(rpt->file, gbusList, HdLd[1], HdRp[1]);
  printHdrUnits(rpt->file, gbusList, HdLd[2], HdRp[2]);
  printHdrBuses(rpt->file, gbusList, step, HdLd[3], HdRp[3]);
  rpt->line += 4;
  return;
}
void printHdrUnits(FILE *fp, Link *curLink, char *l, char *r)
...
void ff_printQgenPoints(ff_Report *rpt, Link *gbusList, float V)
{
  ...
  fprintf(rpt->file, "  %9.2f |", V);
  for (curLink=gbusList; curLink!=NULL; curLink=curLink->next) {
    curData = (Link *) curLink->data;
    b = (pf_rec *) curData->data;
    fprintf(rpt->file, "%6.1f|", b->s.ACbus.Qgen);
    fprintf(rpt->file, "%6.1f|", b->i.ACbus.Qsch_Qmax - b->s.ACbus.Qgen);
  }
  fprintf(rpt->file, "\n");
  rpt->line += 1;
  return;
...
void de_get_bus(int *stat, pf_rec *busID, float *Qshunt)
{
  ...
  pf_rec_bus(busID, "G");                             /* get target QV bus */
...
void de_get_busList(int *stat, Link *gbusList)
{
  ...
  for (curGbus=gbusList; curGbus!=NULL; curGbus=curGbus->next) {
    gbusLink = (Link *) curGbus->data;
    pf_rec_bus((pf_rec *) gbusLink->data, "G");
    pf_rec_bus((pf_rec *) gbusLink->data, "O");
  }
  return;
}
ff_Report *ff_initReport(char *fs, FILE *fp, int ln, int pg, char *ts)
{
  ff_Report *rpt;
  rpt = (ff_Report *) cf_malloc(sizeof(ff_Report));
  strcpy(rpt->spec, fs);           /* file spec    */
  rpt->file        = fp;           /* file pointer */
  rpt->line        = ln;           /* line number  */
  rpt->page        = pg;           /* page number  */
