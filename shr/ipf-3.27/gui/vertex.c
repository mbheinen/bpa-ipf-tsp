...
\**************************************************************************/
{
  ...
  int     depth; 
  Visual *visual;
  int     share_limit = 140, color_red, color_pixel, count = 0;
...
/*dpy   = XtDisplay(XtParent(wid));
  ...
  if( color.pixel <= share_limit )	/* Need method to get share_limit */
  {
    ...
    while( color_pixel <= share_limit && count < share_limit)
    {
      ...
      color_red += adj;	 		/* try another value for red */
      color.red = color_red;
      color_pixel = XAllocColor( XtDisplay( mw ), wmap, &color );
      count++;
    }
