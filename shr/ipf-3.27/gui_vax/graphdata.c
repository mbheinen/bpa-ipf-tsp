static char sccsid[]="@(#)graphdata.c	20.74 7/6/98";
/***************************************************************************
...

int merge_wp(int *wp_status)
/**************************************************************************\
*  Purpose: does the "MERGE" - takes all coord file buses and looks
*		for match in powerflow bus names.
*
...
\**************************************************************************/
{
  ...
  static DB_STAT stat_coord = D_NOTFOUND;    /* status from coord db access */
  char buskey[256];
  char *pbuskey = buskey;
  char newkey[256];
  ...
  NewLine con;
  ConnectData *pcon;		/* template to deciper pf line */
  int rtncde;
  int cnt;
  int nvtx;
  int idx;
...
#define MAX_BUS_CNT 10
  ...
  bus_cnt = 0;
  do {
  if(D_FOUND != stat_coord)	
  {
    stat_coord = findBusGraphVertexRec( "            ", &pgraphcoordvertexnxt );

    if (stat_coord == D_EOF) {
      printf("WARNING: merge - EOF no first coord record in graph_db\n");
      *wp_status = 0;
      return ((Boolean) TRUE);
    }

    if (stat_coord == D_NOTFOUND)
    {
      if(GraphTypeVertex == pgraphcoordvertexnxt->type &&
         GraphClassVertexBus == pgraphcoordvertexnxt->class )
      {
	stat_coord = D_FOUND; /* NOTE: db_search set at first coord record */
      }
      else
      {
	printf("No Bus vertexes found in graph_db - no merge performed.\n" );
        *wp_status = 0;
        return((Boolean)TRUE);
      }
    }
  }
  assert(D_FOUND == stat_coord);
  /** do this coord vertex **/
  sprintf(newkey, "B     %12.12s   \n", pgraphcoordvertexnxt->vertex_id);
  strcat(pfcmdstr, newkey);

  /** get the next coord vertex **/
  pgraphcoordvertex = pgraphcoordvertexnxt;
  stat_coord=db_next(&graph_db, &pgraphcoordvertex, &pgraphcoordvertexnxt, 
							GRAPH_TYPE_VERTEX_ID);
  /* if key values are out of range stop loop */
  if(D_FOUND == stat_coord) {
    if( pgraphcoordvertexnxt->type != GraphTypeVertex || 
     GraphClassVertexBus != pgraphcoordvertexnxt->class ) {
      stat_coord = D_NOTFOUND;
      *wp_status = 0;
      rtn_stat = (Boolean) TRUE;
    }
    else
    {
      stat_coord = D_FOUND;
      rtn_stat = (Boolean) FALSE;
    }
  }
  else
  {
  ...
  bus_cnt++;
  } while (bus_cnt < MAX_BUS_CNT && stat_coord == D_FOUND);

  strcat(pfcmdstr, ENDOFMSG);
  strcat(pfcmdstr, "\n");
  rtncde = ipc_synch_rw(pfdataret, pfcmdstr);
  if(ipfdebug & DB_GraphMask) {
    fprintf(ipfdbio, "merge_wp: pf bus connection %d request %s\n", 
      rtncde, pfcmdstr);
  }
...
void mergeCoordBase()
/**************************************************************************\
* Purpose: starts the merge work process
*
\**************************************************************************/
{
  ...
  {
    status = 1;
    editor_wpid = XtAppAddWorkProc( app_context, (XtWorkProc)merge_wp, &status);
  }
  else
  {
...
void buildMainDisplay( int mergeFlag )
/***************************************************************************\
* Purpose: Builds the main display of buses, lines, comments, (EVERYTHING!)
*
...
\***************************************************************************/
{
  ...
  switch ( mergeFlag )
  {
    case CoordOnly:      /* just display the coord file */
