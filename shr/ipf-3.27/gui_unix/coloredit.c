...
static unsigned long r255[256];
static unsigned long g255[256];
static unsigned long b255[256];
...
\**************************************************************************/
{
  ...
  static int c_watch[256];	/* list of colorcells to watch out for	*/
...
\*************************************************************************/
  ...
  if( rgb_key != last_rgb_key )
  {
    ...
    {
      case ('R'):
 	for( c=0; c<256; c++ )	/* sliding red - look for green or blue */
        {
	   if( g255[c] == color.green && b255[c] == color.blue )
	     c_watch[++n_watch] = r255[c];	/* this red not allowed */
	}
      break;

      case ('G'):
 	for( c=0; c<256; c++ )	/* sliding green - look for red or blue */
        {
	   if( r255[c] == color.red && b255[c] == color.blue )
	     c_watch[++n_watch] = g255[c];	/* this green not allowed */
	}
      break;

      case ('B'):
 	for( c=0; c<256; c++ )	/* sliding blue - look for red or green */
        {
	   if( r255[c] == color.red && g255[c] == color.green )
	     c_watch[++n_watch] = b255[c];	/* this blue not allowed */
	}
      break;
