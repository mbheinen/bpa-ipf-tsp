...
C
	...
	include 'ipfinc/ownhash.inc'
     ...
     &            reports(2)*20, unit_diff(3,11)*10, temp*10
	integer apdoutbuf, o1, o2, findstr, status, zonintchg,
     &          tempfile, open_file, bldzonint
...
c
c       Build zone interchange arrays
c
        status = bldzonint(status)
        update(1) = 0 
...
c
     ...
     &                    reports(iquery)(1:lastx)) 
              status = zonintchg(in_buffer(ix:), out_buffer, scrfil)
              o1 = 1
              do i = 1, 3
