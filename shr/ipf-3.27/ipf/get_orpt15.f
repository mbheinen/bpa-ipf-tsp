...
C
	...
	include 'ipfinc/blank.inc'
     ...
     &            null*1, linefeed*1
	integer apdoutbuf, o1, o2, findstr, status, deleterpt
        ...
        do while (change_f)
           ...
           length = apdoutbuf(i1, '/REPORTS, SELECT NETWORK_DEL ',
     &                        in_buffer(i1:))
           i1 = i1 + length
           change_f = load_fltr(in_buffer, out_buffer, i1)
           if (change_f) then
              out_buffer(1:1) = null
              ix = findstr (in_buffer, 'NETWORK_DEL')
     &           + lastch ('NETWORK_DEL')
              status = deleterpt(in_buffer(ix:), out_buffer,
     &                           scrfil)
