...
c
  ...
  170   if (numdiff .ge. MAXBUS) go to 230
        ...
        if (bus_match .eq. 0) then
           ...
           if (.not. chkfltr(jarzn(nb1), zone(nb1), '***', 
     &                       base(nb1), '**', 0)) then
c
...
c
     ...
     &               chkfltr(oarzn(m1), oldzone(m2), own2, 
     &                       oldbase(m2), '**', 0)) then
...
c
           ...
           if (.not. chkfltr(oarzn(nb2), oldzone(nb2), '***', 
     &                 oldbase(nb2), '**', 0)) then
c
...
c
     ...
     &            chkfltr(oarzn(m2), oldzone(m2), own2, 
     &                    base(m1), '**', 0)) then
...
10255               format (f8.2, f10.2, f10.2)
  ...
  294         format(1x, a2, 1x, a3, 1x, a8, a4, 1x, a8, a4, 1x, a1,
     &               i1, t43, f8.5, t57, f8.5, t66, f8.5, 1x, a3)
           endif
           if (rpt_type .eq. 17) then
              if (loop(2) .eq. 0) then
...
10291         format(1x, a2, 1x, a3, 1x, a8, a4, 1x, a8, a4, 1x, a1,
     &               i1, t43, a, t73, a, t104, f8.2, 1x, a3)
              if (loop(2) .eq. 1) then
                 if (o2 .lt. maxbuf_out) then
                    last = lastch(text)
