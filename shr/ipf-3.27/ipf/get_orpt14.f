C    @(#)get_orpt14.f	20.4 1/7/99
C****************************************************************
C
C       File: get_orpt14.f
C       Purpose: Routine to generate change list output 
C                reports using IPF command
C
C                / REPORTS, SELECT NETWORK_CHA
C                (END)
C
...
C
	...
	include 'ipfinc/blank.inc'
     ...
     &            null*1, linefeed*1
	integer apdoutbuf, o1, o2, findstr, status, chglisrpt
        ...
        do while (change_f)
           ...
           length = apdoutbuf(i1, '/REPORTS, SELECT NETWORK_CHA ',
     &                        in_buffer(i1:))
           i1 = i1 + length
           change_f = load_fltr(in_buffer, out_buffer, i1)
           if (change_f) then
              out_buffer(1:1) = null
              ix = findstr (in_buffer, 'NETWORK_CHA')
     &           + lastch ('NETWORK_CHA')
              status = chglisrpt(in_buffer(ix:), out_buffer,
     &                           scrfil)
