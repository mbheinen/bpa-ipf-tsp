C    @(#)chglod.f	20.5 11/12/98
      subroutine chglod (nwrd, word, pctp, pctq, pltoto, qltoto,
...
C     process %LOAD_CHANGE within /CHANGE_PARAMETERS
C  
...
C                                         Print out results
  ...
  370 format ('0 %LOAD_CHANGE, PL = ', f6.1, '%, QL = ', f6.1, '%') 
      call prtout (1)   
  ...
  430    continue
     ...
     1            zone(nb), owner(nb), plo, qlo, busdta(3,nb),  
     2            busdta(4,nb)  
...
C                                                    
  480       plo = bctbl(2,icb)
     ...
     1               plo, qlo, bctbl(2,icb), bctbl(3,icb)   
  ...
  510    continue   
         if (dpltot .ne. 0.0 .or. dqltot .ne. 0.0) then 
            ...
            ploadu(kt) = ploadu(kt) + dpltot   
            qloadu(kt) = qloadu(kt) + dqltot  
            ntbx = ikk(3,nb)
            if (ntbx .gt. 0) then   
               ltyp = tbx(1,ntbx)  
  ...
  530 format ('0', 2x, 'TOTAL', t28, 'ORIGINAL LOAD', 2f9.2,
     1   7x, 'FINAL LOAD', 2f9.2)   
      call prtout (1)   
