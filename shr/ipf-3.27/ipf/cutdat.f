C    @(#)cutdat.f	20.4 6/27/97
      subroutine cutdat (nsyst) 
...
c
  ...
  100          format(' More than ', i5, ' Y-matrix entities')
            ...
            do l = 1, ls
               ...
               gkmu(l+yptr) = gkmu(l+ln)
               bkmu(l+yptr) = bkmu(l+ln)
            enddo
            ikmu(ls+yptr+1) = mt
            gkmu(ls+yptr) = 0.0
            bkmu(ls+yptr) = 0.0
...
C     Pi-back buses
  ...
  440 call forbtm   
      write (outbuf,450)
  450 format ('0 Input listing of PI-BACK BUSSES '  )   
      call fortop   
      call prtout(1)
      call space(2) 
      do 460 i=1,idat   
...
C       
  ...
  850                   format(1x,a8,f6.1,7x,a8,f6.1)
      ...
      enddo
C       
C     Check viability of PI-BACK BUSES  
C       
      kpibak = 0
      do ix = 1, ntot_alf
         i = alf2inp(ix)
         if (ikk(4,i) .eq. 1) then 
            if (ikk(1,i) .eq. 0) then  
               write (errbuf(1),882) bus(i),base(i)
  882          format (' Pi-back bus ',a8,f7.1,
     &           ' is ignored because it is in cut network.')  
               call prterx ('W',1) 
               ikk(4,i) = 0
            else   
               n = 0   
               ...
               do l = lf, ls
                  m = kolum(l)
                  if (ikk(1,m) .eq. 1) n = n + 1  
               enddo
               if (n .gt. 2) then  
                  write (errbuf(1),900) bus(i),base(i),n   
  900             format (' Pi-back bus ',a8,f7.1,' has ',i2,
     &               ' branches and is ineligible for Pi-back.') 
                  call prterx ('W',1)  
                  ikk(4,i) = 0 
               else
                  if (kpibak .ge. MAXPIBUS) then   
                     write(errbuf(1),910) bus(i),base(i), MAXPIBUS
  910                format(' Pi-back bus ',a8,f7.1,' exceeds ',
     &                  i4, '. Bus ignored.')   
                     call prterx ('W',1)   
                     ikk(4,i) = 0  
                  else 
...
C       
  ...
  950       format('0 ',i2,
     &        ' Separate cut sub-networks will be punched.')  
            call fortop
            call prtout (1)
            call space (2) 
         endif 
 ...
 1010    format(  35x,'"E" pertains to envelope node ' )   
         call prtout(1)
         write (outbuf,1020)   
 1020    format(35x,'"P" pertains to pi_back buses')   
         call prtout(1)
         call space(2) 
         knt=0 
         do ix = 1, ntot_alf
