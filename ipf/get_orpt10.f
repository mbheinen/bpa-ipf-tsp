C    @(#)get_orpt10.f	20.4 1/7/99
C****************************************************************
C
C       File: get_orpt10.f
C       Purpose: Routine to generate tie line output 
C                reports using IPF command
C
C                / REPORTS, SELECT TIE_LINE
C                  WHERE BUS = "<busname>"
C                (END)
C
...
C
	...
	include 'ipfinc/blank.inc'
     ...
     &            null*1, linefeed*1
	integer apdoutbuf, o1, o2, findstr, inttierpt, status
        ...
        do while (change_f)
           ...
           length = apdoutbuf(i1, '/REPORTS, SELECT TIE_LINE ',
     &                        in_buffer(i1:))
           i1 = i1 + length
           change_f = load_fltr(in_buffer, out_buffer, i1)
           if (change_f) then
              out_buffer(1:1) = null
              ix = findstr (in_buffer, 'TIE_LINE')
     &           + lastch ('TIE_LINE')
              status = inttierpt(in_buffer(ix:), out_buffer, scrfil)
