C    @(#)get_otrpt1.f	20.4 1/7/99
C****************************************************************
C
C       File: get_otrpt1.f
C       Purpose: Routine to generate bus-branch output reports
C                using IPF command
C
C                / REPORTS, SELECT BUS_BR_OUTPUT 
C                  WHERE BUS = "<busname>"
C                (END)
C
C       Author: Walt Powell  Date: 12 April 1993
C                            Modified:
C       Called by:
C
C****************************************************************
C
	subroutine get_otrpt1 (scrfil)
        integer scrfil
  ...
  216      format (i1)
     ...
     &              call shellsrt (first, last, komp_buf, swap_buf)
                 do iv = first, last
                    ...
                    length = apdoutbuf(i1, 
     &                 '/REPORTS, SELECT BUS_BR_OUTPUT WHERE BUS = '
     &                 // '"' // text(7:18) // '"' // linefeed 
     &                 // '(END)', 
     &                 in_buffer(i1:))
                    out_buffer(1:1) = null
     ...
     &                 + lastch ('BUS_BR_INPUT')
                    call busbrotrpt(in_buffer(ix:), out_buffer, 
     &                              scrfil)
