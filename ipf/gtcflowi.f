C    @(#)gtcflowi.f	20.10 1/4/99
C****************************************************************
C
C   File: gtcflowi.f
C   Purpose: Routine to obtain BCD image of CFLOW-requested data --
c            handles "GET_DATA,TYPE=CFLOW_INPUT,TABLE=xxxx,ACTION=xx"
C
...
C****************************************************************
C
        integer function gtcflowi (in_buffer, out_buffer)
...
c
     ...
     &            save_br_type*2
        integer find_bus, apdoutbuf, find_ara, ftn_atoi
        integer next_brp, frst_brp, lastch, nxt_term, ptr, o2, 
     &          loop_ai(3), loop_b1, loop_cb, loop_q, loop_b2, 
     &          loop_brp, ic, nb, i, ieq, kxd, next, nwrd, iwrd, i1, 
     &          lctext, k1, k2, nc, ncb, kpqd, ib, length, jb
   ...
   11      format(' Request rejected - no base data loaded' )
        ...
        gtcflowi = 0  ! Set default return status "successful"
...
c
        if ( table .eq. 'AREA'  .or.  table .eq. 'ITIE'  ) then
           ...
           elseif ( table .eq. 'ITIE' ) then
              ...
              nc = 1
              do while (nc .le. ntotic .and. loop_ai(3) .eq. 0)
                 if (arcint(1,nc) .eq. arcnam(k1) .and.
     &               arcint(2,nc) .eq. arcnam(k2)) then
                    loop_ai(3) = nc
                 endif
                 nc = nc + 1
              enddo
...
c
  200   continue
        if (table .eq. 'AREA') then
           ...
           loop_ai(1) = ib
           if ( ib .gt. 0  .and.  ib .le. ntotc ) then
  ...
  250            format (' Output buffer overflowed at text ', a)
        ...
        elseif (table .eq. 'ITIE') then
           ...
           loop_ai(3) = ib
           if ( ib .gt. 0  .and.  ib .le. ntotic ) then
        ...
        elseif (table .eq. 'BUS ') then
           ...
           loop_b1 = ib
           if ( ib .gt. 0  .and.  ib .le. ntot_alf ) then
              k1 = alf2inp(ib)
     ...
     &            ntot2 .gt. 0  ) then
              ...
              do while ( ib .le. ntot_alf  .and.  .not. found ) 
                 k1 = alf2inp(ib)
                 ncb = kbsdta(15,k1)
                 if (ncb .gt. 0) then
                    found = .true.
