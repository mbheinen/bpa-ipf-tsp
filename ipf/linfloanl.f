C    @(#)linfloanl.f	20.6 5/27/98
     ...
     &          outfile*60, query*1, cutfile*60, version*10
      logical loadbase, loaded, finished, error, gen_flag, excel_flag
      integer status, open_file, scrfil, find_bus, ptr, sen_cut,
     &        open_apnd, buildzbo, kmpvltdif
...
C     Define logical units
      ...
      write (*, 10000) version
10000 format (' > Begin execution of Path Use Factors version ', a)
...
C     Get Powerflow File Name
      ...
      do while (.not. loaded)
        write (*, 10010)
10010   format (' > Enter powerflow BASE file name : ')
...
C
      write(*, 10040)
10040 format(' > Enter Excel flag (Y or N): ')

      read (*, fmt='(a)') text(1:1)
 
      excel_flag = (text(1:1) .eq. 'Y' .or. text(1:1) .eq. 'y' .or.
     &              text(1:1) .eq. ' ' .or. text(1:1) .eq. char(0))
...
c                       ! 0.1 degree.
        ...
        text(1:1) = '1'
        write(*, 10060)
10060   format(' * Area Interchange Options: 0 = Off ', /,
     &         '                             1 = On ', /,
     &         ' > Enter selection: ', $)
        read (*, fmt='(a)') text
...
C     Get Cutting control File Name
      ...
      do while (.not. loaded)
        write (*, 10070)
10070   format (' > Enter CUT command file name or <RETURN>: ')

        read (*, fmt='(a)') cutfile
 
        if (cutfile .eq. ' ' .or. cutfile(1:1) .eq.  char(0)) then
          loaded = .true.
          luncut = 0
        else
...
10240   format (' * Data error in record (', a, ')')
  ...
  101 continue
      ...
      write (*, 10250) 
10250 format (' > Do you wish to confine the listing to generators only 
     &(Y or N): ')
      query = 'Y'
      read (*, fmt='(a)') query
      gen_flag = (index ('Yy', query) .ne. 0) 
