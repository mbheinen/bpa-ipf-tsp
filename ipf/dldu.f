C    @(#)dldu.f	20.6 7/18/96
      subroutine dldu (k1,m1,y1,dydb,deltax,dgdu)
C
C     This subroutine computes the sensitivity dLoss/dBkl.
C
...
C
      ...
      pij = real (s(1)) * bmva
C
C     Compute dG/dU.  This will be non-zero only if Bkl is connected to
C     system or area slack bus.
C
      dgdu = 0.0
      if (kspare(20) .eq. 0) then
         ...
         if (jflag(1,i) .eq. 3) then
            dgdu = dgdu + real (v(1)*conjg(v(1)) * dydb(1,1))
   ...
   80    continue
         ...
         do 90 i = i1,i2
         if (jflag(1,i) .eq. 3) then
            dgdu = dgdu + real (v(2)*conjg(v(2)) * dydb(2,2))
     1                    + real (v(1)*conjg(v(2)) * dydb(2,1))
         endif
   90    continue
      else
