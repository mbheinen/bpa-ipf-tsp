static char sccsid[]="@(#)pscordat.c	20.75 6/4/98";
/*****************************************************************************\
* file: pscordat.c
* author: Dan Clark
* date: Dec 29, 1992
...
void modify_bus_angle	  ( GraphElement *, char *,  char *, GraphClass, int );
...
\***************************************************************************/
{
  ...
  if( curseg != -1 )            /* if -1 then don't show branch flow */
  {				/* (and don't change it)	     */
...
/*  list_comment_strs(); */
      ...
      vertex->class == GraphClassVertexName )
  {
    ...
    XtSetSensitive( widget_id("dialog_finished_button"), TRUE );
  }
	...
	/* VertexComment, VertexGroup, etc.				    */
  {
    ...
    XtSetSensitive( widget_id("dialog_finished_button"), FALSE );
  }
...
\****************************************************************************/
{
  ...
  if( D_FOUND == stat )
  {
    ...
    findVertxLinkedToVertx( &pgraphobj, pe, GraphClassVertexCapacitor );
    ...
    sprintf( str, "%3.0f", ang);
    lookup_and_fill_field( "shunt_angle_text", str, 0, 2 );
  ...
  else		/* set default values */
  {
    ...
    lookup_and_fill_field( "shunt_angle_text", "  0", 0, 2 );
    set_button_state( "show_bus", True );
...
\************************************************************************/
{
  ...
  switch ( which_display )
  {
    ...
    lookup_and_get_field( "shunt_angle_text" , angle, 4 );
    add_blanks_if_short( angle, 4, "shunt_angle_text" );
    reformat (angle, angle, "f3.0");
...
\****************************************************************************/
{
  ...
  if( D_FOUND == stat )
  {
    ...
    lookup_and_get_field( "bus_x_pos_text", str, 7 );
    x = pscoord_to_MOTIF_x( str, 6, pgraphbus->vertex_id );
    lookup_and_get_field( "bus_y_pos_text", str, 7 );
    y = pscoord_to_MOTIF_y( str, 6, pgraphbus->vertex_id );

    if( pgraphbus->x != x || pgraphbus->y != y )
    ...
    lookup_and_get_field( "name_x_pos_text", str, 7 );
    x = pscoord_to_MOTIF_x( str, 6, pgraphbus->vertex_id );
    lookup_and_get_field( "name_y_pos_text", str, 7 );
    y = pscoord_to_MOTIF_y( str, 6, pgraphbus->vertex_id );
    ...
    modify_bus_angle( pgraphbus, "generator_angle_text", "show_gen",
				  GraphClassVertexGenerator, G_DIST );
    modify_bus_angle( pgraphbus, "shunt_angle_text",	 "show_reac",
				  GraphClassVertexCapacitor, R_DIST );

  }
}

void modify_bus_angle( GraphElement *pgraphbus, char *angle_text,
...
\**************************************************************************/
{
  ...
  int c_ang, d_ang;
  char str[16];
  char new_hide_status;

  lookup_and_get_field( angle_text, str, 4 );
  c_ang = (int)atof_cnt_zero( str, 3 );

  if ( (int)XmToggleButtonGetState(widget_id(show_box)))
  ...
  if( pgraphobj == NULL && class == GraphClassVertexCapacitor )
  {
    findVertxLinkedToVertx( &pgraphobj, pgraphbus, GraphClassVertexReactor );
  }

  if( pgraphobj )
  {
    d_ang = (int)(radiansBetweenPts( pgraphbus->x, pgraphobj->y,
				pgraphobj->x, pgraphbus->y )/DegtoRad + .5);
    ...
    if( c_ang != d_ang || pgraphobj->hide != new_hide_status )
    {
      ...
      pgraphnode->x=pgraphbus->x+(int)(cos(c_ang*DegtoRad)*dist*zoom_factor+.5);
      pgraphnode->y=pgraphbus->y-(int)(sin(c_ang*DegtoRad)*dist*zoom_factor+.5);
    ...
    if( new_hide_status != pgraphobj->hide )
    {
	...
	case NO_HIDE:
	  manageVertex(pgraphobj);	/* gen/reac vertex */
    ...
    if( pgraphobj->display == GraphDisplayOn )
    {
...
\********************************************************************/
{
  ...
  else if ( class==GraphClassVertexCapacitor )
     strcpy( name, SHUNT_N );
...
\**************************************************************************/
{
  ...
  {
    pnew->hide = HIDE;
    lookup_and_fill_field( "generator_angle_text", "  0", 0, 3 );
  }
  else
  {
    pnew->hide = NO_HIDE;
    lookup_and_fill_field( "generator_angle_text", "  2", 0, 3 );
  }
...
\**************************************************************************/
{
  ...
  if( pgen == NULL )
    findVertxLinkedToVertx   ( &pgen, pe, GraphClassVertexCapacitor );
  ...
  {
    pnew->hide = HIDE;
    lookup_and_fill_field( "shunt_angle_text", "  0", 0, 3 );
  }
  else
  {
    pnew->hide = NO_HIDE;
    lookup_and_fill_field( "shunt_angle_text", "  2", 0, 3 );
  }
