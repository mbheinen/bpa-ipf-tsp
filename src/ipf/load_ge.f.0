...
c
        ...
        options(4) = 1        ! Default TX Bottleneck = RateA
        options(5) = 1        ! Default Ln Nominal = RateA
        options(6) = 1        ! Default Ln Thermal = RateA
        options(7) = 2        ! Default Ln Bottleneck = RateB
          ...
          if ( firstxstr(word(iwrd), 'FILE') .ne. 0)  then
            ...
            do while (found)
              if ( firstxstr(word(iwrd), 'T*') .ne. 0)  then
                ...
                last = lastch (tempc)
                do i = 1, last
                  options(i) = index ('ABCDEFGH', tempc(i:i))
                enddo
                iwrd = iwrd + 1
                ...
                last = lastch (tempc)
                do i = 1, last
                  options(4+i) = index ('ABCDEFGH', tempc(i:i))
                enddo
                iwrd = iwrd + 1
              else
                found = .false.
...
c
c             Skipped data in raw data file
C
              count = ftn_atoi(word(4))
              finished = .false.
              start = numrec
