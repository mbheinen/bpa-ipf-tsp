C    @(#)get_otrpt5.f	20.5 1/7/99
C****************************************************************
C
C       File: get_otrpt5.f
C       Purpose: Routine to generate overloaded lines reports
C                using IPF command
C
C                / REPORTS, SELECT OVERLOADED_LINES
C                  WHERE AREAS = ...
C                (END)
C
...
C
	...
	integer apdoutbuf, o1, o2, findstr
        ...
        do while (change_f)
           ...
           length = apdoutbuf(i1, '/REPORTS, SELECT OVERLOADED_LINES ',
     &                        in_buffer(i1:))
           i1 = i1 + length
...
c
     ...
     &                   // linefeed // '(END)',
     &                   in_buffer(i1:))
                    ix = findstr (in_buffer, 'OVERLOADED_LINES')
     &                 + lastch ('OVERLOADED_LINES')
                    call ovldlnsrpt(in_buffer(ix:), out_buffer, scrfil)
                    o1 = 1
                    do i = 1, 4
     ...
     &                          .and. no_more) then
                                ...
                                i1 = index (in_buffer, null)
                                length = apdoutbuf (i1, 
     &                   '/REPORTS, SELECT OVERLOADED_LINES CONTINUE',
     &                             in_buffer(i1:))
                                i1 = i1 + length
                                out_buffer(1:1) = null
                                call ovldlnsrpt(in_buffer, out_buffer, 
     &                             scrfil)
                                o1 = 1
                                do i = 1, 4
                                   o2 = nxt_term(out_buffer(o1+1:)) + o1
