...
int pf_del_area(char *area) {
   strcpy(data,"DA "); strcat(data,area); query_pf(Changes,data);
   return ipf_status;
}
int pf_del_zone(char *zone) {
   strcpy(data,"DZ "); strcat(data,zone); query_pf(Changes,data);
   return ipf_status;
}
int pf_rename_area(char *oldname, char *newname) {
   sprintf(data,"ZA %-10.10s  %-10.10s",oldname,newname); query_pf(Changes,data);
   return ipf_status;
}
...
int pf_rename_zone(char *oldname, char *newname) {
   sprintf(data,"Z  %-2.2s%-2.2s",oldname,newname); query_pf(Changes,data);
   return ipf_status;
}
int pf_load_changes(char *filename) {
   char cmd[128];
   strcpy(cmd,"/changes, file="); strcat(cmd,filename);
   query_pf(cmd,"");
   return ipf_status;
}
int pf_load_netdata(char *filename) {
   char cmd[128];
   strcpy(cmd,"/network_data, file="); strcat(cmd,filename);
   query_pf(cmd,"");
   return ipf_status;
}
int pf_load_oldbase(char *filename) {
   char cmd[128];
   strcpy(cmd,"/old_base, file="); strcat(cmd,filename);
   query_pf(cmd,"");
   return ipf_status;
}
...
int pf_rec_area(pf_rec *r, char *action) {
   ...
   switch(toupper(action[0])) {
      ...
      case 'G': cv_pf_b2a(data,r);
                if (query_pf(cmd,data)==0) cv_pf_a2b(r,reply_pf);
                break;
      case 'D': cv_pf_b2a(data,r); data[2]='D'; query_pf(Changes,data); break;
      case 'A': cv_pf_b2a(data,r); query_pf(Changes,data); break;
      case 'M': cv_pf_b2a(data,r); data[2]='M'; query_pf(Changes,data); break;
...
int pf_rec_comments(pf_comments *r, char *action) {
   ...
   switch(toupper(action[0])) {
      ...
      case 'M': sprintf(data,"case_id=%s\ncase_ds=%s\nH %s\nH %s\n",
                ...
                query_pf(PutComments,data); break;
