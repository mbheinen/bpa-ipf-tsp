...
#define ENDOFMSG "*[EOM]"
#define SEPCHARS "\n"
static char outbuf_array[4096];
static char inbuf_array [4096];
...
/*   while( psubstr != NULL )
   {
     if( !strncmp(psubstr, ENDOFMSG, sizeof(ENDOFMSG))) { break; }
  ...
  if( *(psubstr+1) != 'D' && *(psubstr+1) != 'M' )
  {
    stringPart( psubstr, str, 42, 46 );
