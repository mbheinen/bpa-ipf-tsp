C    @(#)linfloanl.f	20.6 5/27/98
      subroutine linfloanl(f_argc, f_argv)
      integer f_argc
      character f_argv(*)*60
     ...
     &        open_apnd, buildzbo, kmpvltdif, p_level, ftn_atoi
      ...
      iadd = locint (f_argc)
      write (*, 10000) iadd
10000 format (' main_f.f: &argc = [', z8, ']')

      i = 1
      do while (i .le. f_argc)
        iadd = locint (f_argv(i))
        length = lastch (f_argv(i))
        write (*, 10001) i, iadd, f_argv(i)(1:length)
10001   format (' main_f.f: argv[', i2, '] = &[', z8, '], [', a, ']')

        if (f_argv(i)(1:2) .eq. '-h' .or. f_argv(i)(1:2) .eq. '-H') then
          write (*, 10003)
10003     format (' Usage: LFA [-options] ', /,
     &      '  Options   Description                       Default ', /,
     &      '  -X<flag>  Excel delimiting character        |       ', /,
     ...
     &      '  -H        Help (This printout)              (none)  ', /)
        ...
        else if (f_argv(i)(1:2) .eq. '-x' .or. 
     &           f_argv(i)(1:2) .eq. '-X') then 
C
C         Set Excel flag
C
          excel_flag = .true.
        ...
        else if (f_argv(i)(1:2) .eq. '-a' .or. 
     &           f_argv(i)(1:2) .eq. '-A') then 
C
C         Set Area Interchange Constraint flag
C
          ai_flag = .true.

        else if (f_argv(i)(1:2) .eq. '-g' .or. 
     &           f_argv(i)(1:2) .eq. '-G') then 
C
C         Set Generator flag
C
          gen_flag = .true.

        else if (f_argv(i)(1:2) .eq. '-p' .or. 
     &           f_argv(i)(1:2) .eq. '-P') then 
C
C         Set Phase shifter sensitivity flag
C
          ps_flag = .true.

        else if (f_argv(i)(1:2) .eq. '-l' .or. 
     &           f_argv(i)(1:2) .eq. '-L') then 
C
...
C
        ...
        else if (f_argv(i)(1:2) .eq. '-c' .or. 
     &           f_argv(i)(1:2) .eq. '-C') then 
C
C         Set Cut flag
C
          cut_flag = .true.
