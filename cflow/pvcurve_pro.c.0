/********** Marker ****************/
/**** testing saved pre outage basecases scheme ********/  
/******************************************************************************\
...
SEE ALSO:   QVcurve
UPDATED:    March 10, 1999 (see PV_Version below)
LANGUAGE:   Standard C.  CFLOW Libraries.  CF_UTIL.H.
DEVELOPER:  David M. Stefonek   (360)418-8823   dmstefonek@bpa.gov
REQUESTER:  Mark Bond
USERS:      J Randall, S Kinney, K Kohne, M Rodrigues, G Comegys, B Tesema, C Matthews,.
IPF:        Version 322 or above recommended.
...
#define  QUERY_BLIM  (int) (0<< 6) /* DO NOT prompt for BUS PERTURBATION limits */
#define  QUERY_LLIM  (int) (0<< 7) /* DO NOT prompt for LOAD PERTURBATION limits */
...
#define  QUERY_PRTRB (int) (1<<23) /* prompt for PERTURBATION FILE */
#define  QUERY_STPS  (int) (1<<24) /* prompt for steps beyond critical outage(trace->lmt_sol) */
#define  QUERY_FSF   (int) (1<<25) /* prompt for File Save Flag (trace->FilSavFlg) */
...
#define  PERTURB_F   (int)      3  /* LOAD_PERTURBATION FROM FILE         = 3 */
...
typedef struct traceRecord {
  Link  *baseList;
  Link  *baseListTmp; /* structure of temp base files including perturbation Id*/
  Link  *pchgList;
  Link  *brchList;
  Link  *comoList;
  Link  *prtrbList;/* perturbation list identifiers from perturbation file */
  ...
  int    stat;
  long   query;
  int    perturb; /* BUS_PERTURBATION = 1, LOAD_PERTURBATION = 2 , PERTURBATION from FILE = 3*/
  ff_Report *trc;
  FILE  *prtrbFile;
  FILE  *comFile;
  ...
  char   prtrbSpec[FILENAME_MAX];    /* File with perturbation list */
  ...
  int    curvNumber;
  char   FilSavFlg[2];      /* Save Pre*.bse Files: A-All, C-Critical, N-None (default) */
  char   FilNam1[80];    /* Name of Critical Pre*.bse File to be saved */
} Trace;
/************************ cf_print structure ******************************/
typedef struct {
  int   cut_plane;
  int   head_cnt;         /* print only one heading on excel files */
  int   max_sol;          /* maximum number of post outage solutions expected  */
  int   cnt_sol;          /* count of post outage solutions for a perturbation */
                          /* initally set to 10000, should be set to number of */
                          /* pre outage solution perturbations.                */
  char   FilNam1[80];     /* Name of Potential Critical Pre*.bse File to be saved */
                          /*  as identified in de_pv_curve()                      */
...
void   ff_printDvdqHeader(FILE *fp, Link *mbusList);

/* void   ff_printDvdqPoints(FILE *fp, int perturb, int stat, Link *mbusList, float P, float T); */
 void   ff_printDvdqPoints(FILE *fp, int perturb, int stat, Link *mbusList, char *PrtrbId); 

/* void   ff_printVoltHeader(FILE *fp, Link *mbusList, Link *pbusLink); */
void   ff_printVoltHeader(FILE *fp, Link *mbusList);

/* void   ff_printVoltPoints(FILE *fp, int perturb, int stat, Link *mbusList, float P, float T); */
 void   ff_printVoltPoints(FILE *fp, int perturb, int stat, Link *mbusList, char *PrtrbId);

/* void   ff_printLoadHeader(FILE *fp, Link *mbusList, Link *pbusLink); */
void   ff_printLoadHeader(FILE *fp, Link *mbusList);

/* void   ff_printLoadPoints(FILE *fp, int perturb, int stat, Link *cutpList, float P, float T); */
 void   ff_printLoadPoints(FILE *fp, int perturb, int stat, Link *cutpList, char *PrtrbId); 

/* void   ff_printQresHeader(FILE *fp, Link *gbusList, Link *pbusLink); */
void   ff_printQresHeader(FILE *fp, Link *gbusList);

/*void   ff_printQresPoints(FILE *fp, int perturb, int stat, Link *gbusList, float P, float T); */
void   ff_printQresPoints(FILE *fp, int perturb, int stat, Link *gbusList, char *PrtrbId); 
...
void   ff_printHeadrNamesPB(FILE *fp, char *l, char *r, Link *gbusList);
void   ff_printHeadrBuses(FILE *fp, char *l, char *r, Link *gbusList);
void   ff_printLdHdrNames(FILE *fp, char *l, char *r, Link *cutpList);
...
/***************** seperate pre and post outage  *************/
void   de_pv_curve_pre(Trace *trace, Step *step);
/***************** seperate pre and post outage  *************/
...
void   de_save_newbase(int *stat, char *file);
...
void   de_apply_perturbation(char *PrtrbId, FILE *prtrbFile);
...
/***************** Link a structure *****************/ 
Link *ff_FileFlt22Link(char *file, float flt1, float flt2);
Link *ff_StrStrToLink(char *str1, char *str2);
/***************** Link a structure *****************/ 

void   de_set_gen(int *stat, Link *pbusLink, float Pstart, float Pstop, float Xstart);
...
int rand_file(char *FilNam);
void blank_2_undrbr(char *str);
void scrunch_blanks(char *buf_src, char *buf_cmp, int buf_size);
...
void  initializeTrace(Trace *trace)
{
  ...
  trace->query            = ( QUERY_BASE | QUERY_BRCH | QUERY_PBUS |
                              ...
                              QUERY_SERI | QUERY_PRTRB| QUERY_STPS |
                              QUERY_FSF);
  trace->solution[0]         = '\0';
  trace->change_bus_types[0] = '\0';
  trace->perturb         = PERTURB_F; 
  trace->Pstart          = 0.00;
  trace->Pstop           = 75.0;
  trace->Pstep           = 25.0;
  trace->Ppct            = 5.0;
...
void processCommandLine(Trace *trace, int argc, char *argv[])
/* note: could generalize by passing in a defaultlist to use */
{
  Link *list, *topLink;
  int   i, start = 1;
  char c;
...
/***   if (vn != FF_IPF_VERSION) {*** Don't Check, just verif the versions in use ****/
    printf("\a\n - - - - - - - - - VERSION NOTIFICATION  - - - - - - - - - - - -\a\n");
    printf(" You are using PVcurve version %d.\n",FF_PVCURVE_VERSION);
    ...
    printf("\a - - - - - - - - - - - - - - - - - - - - - - - - - - - - -\a\n");
/***  } *** Don't Check, just verif the versions in use ****/
}
void promptUser(Trace *trace)
{
  char query[MAX_IN];
...
**************** Eliminate Control File Option ****************/
  if (trace->query & QUERY_PERT) {
    ...
    printf("\n   1. BUS Perturbation:  2. LOAD Perturbation;  3. File Perturbation");
...
/************ Check only if BUS PERTURBATION ***************/
  if (trace->perturb == PERTURB_B)
  {  /**** BEGIN PERTURBED BUS DATA COLLECTION ****/
  ...
  }  /**** END PERTURBED BUS DATA COLLECTION ****/

/************ Check only if BUS PERTURBATION ***************/
  if (trace->query & QUERY_PRTRB) {
    printf("\n\n/REPORT");
    cf_sprompt("\n > Enter Perturbation file, PERTURBATION_FILE=: ", "", trace->prtrbSpec);
  }
  if (strlen(trace->prtrbSpec)==0) 
  {
    trace->query &= ~(QUERY_PRTRB);
  }

  if (trace->query & QUERY_FSF) {
    printf("\n Specify the Pre_outage.bse files to be saved for later analysis");
    printf("\n   A. All:  C. Critical:  N. None (default)");
    cf_sprompt("\n > Enter choice, File_Save_Flag = [%s]: ", trace->FilSavFlg, trace->FilSavFlg); 
  }
  if (strlen(trace->FilSavFlg)==0) { 
    trace->query &= ~(QUERY_FSF);
    }
  else
    {
    cf_str2upper(trace->FilSavFlg);
    }
...
************** forget the BRANCH stuff ***********/
  ...
  if (trace->query & QUERY_STPS) {
    cf_iprompt("\n > Enter number of steps to solve beyond critical outage: [%d]", trace->lmt_sol, &trace->lmt_sol);

 /*cf_iprompt("\n > Number of steps,            LOAD_STEPS =  [%2d] : ",trace->Pnum, &trace->Pnum);*/

  }

  if (trace->pbusList==NULL && trace->stepList==NULL && trace->prtrbSpec==NULL) {
...
void  getPrtrbList(Trace *trace)
{
  ...
  else if ((trace->prtrbFile=fopen(trace->prtrbSpec, "r"))==NULL) {
    cf_logErr("Skipping Perturbation File Data: Cannot find %s\n", trace->prtrbSpec);
    trace->prtrbList = NULL;
  }

/****  else if (trace->comoList==NULL && trace->stepList==NULL) {  ****/

  else if (trace->prtrbList==NULL) {  
    rewind(trace->prtrbFile);
    while ( fgets(s, MAX_IN, trace->prtrbFile) != NULL ) {
        cf_str2upper(s);
        if ( s[0] != '>' ) continue;              /* skip comments, skip data */
        if ( strstr(s, "PERTURBATION_FILE")!=NULL ) continue;
        if ( strstr(s, "PERTURBATION ")==NULL ) continue;

/*** This is tricky format  
 ***                      !  Skip the first string (MODE) 
 ***                             ! Negation flag
 ***                              !! Accept everything except a new line 
 ***/     
        if ( sscanf(s, ">%*s %80[^\n]", c) == 1 ) { 
            printf(" Found Perturbation: {%s}\n", c);
            curPrtrb = cf_text2Link(c);
            cf_appList(&trace->prtrbList, curPrtrb);
        }
    }
  }
}
...
void  getCoMoList(Trace *trace)
{
  ...
  if ( strlen(trace->comSpec)==0 ) {
    ...
    while ( fgets(s, MAX_IN, trace->comFile) != NULL ) {
        ...
        if ( sscanf(s, ">%*s %80[^\n]", c) == 1 ) {
            printf(" Found Common Mode Outage: {%s}\n", c);
...
void  assembleSteps(Trace *trace)
{
/*** NOTE: references to prtrb are not complete and may not be necessary ****/
  Link   *stepLink, *curBase, *curPchg, *curBrch, *curComo, *curPrtrb;
  Step   *step;
  int     base, chng, bran, como, outg, prtrb;
...
/* test mimic function : Loop through all cases in base case list */
  ...
  curPrtrb = trace->prtrbList;
  base = 0; chng = 0; bran = 0; como = 0; outg = 0, prtrb;
  while (curBase != NULL) {
    ...
    if (trace->usrSpec[0]!='\0') {
        sprintf(step->preUA,"%s%d%d%c%dUA.TXT", trace->outSeri, base, chng, '_', outg);
...
/************ begin pre outage solutions *******************/
    ...
    cleanUpFiles(trace);
    de_pv_curve_pre(trace, step);

/************ begin post outage solutions *******************/
  cnt_sol = 10000;
  for (stepLink = trace->stepList; stepLink != NULL; stepLink = stepLink->next){
    ...
    if (data_chk.cnt_sol == 0) strcpy (data_chk.FilNam1, " ");  
    if (data_chk.cnt_sol < cnt_sol)
     {
      cnt_sol = data_chk.cnt_sol;
      strcpy (trace->FilNam1, data_chk.FilNam1); 
     }
  }

/*********** Delete Pre *.bse files generated by this study *************/
  if (strcmp(trace->FilSavFlg, "A"))
  {
     TmpBaseList = trace->baseListTmp;
     if (!strcmp(trace->FilSavFlg, "N"))trace->FilNam1[0] = '\0';

     while (TmpBaseList != NULL)
     {
        PreFile_Id = (cf_StrStr *) TmpBaseList->data;
      
/************** WORKING HERE **************/
     if (strcmp(PreFile_Id->str1, trace->FilNam1))
        {
        sprintf(cmd,"/syscal\ndel/noconfirm %s.*\n", PreFile_Id->str1); 
        printf("  %s\n", cmd);
        pf_command(cmd);
        }
     TmpBaseList = TmpBaseList->next;
/************** WORKING HERE **************/
     }
  }
/*********** Delete Pre *.bse files generated by this study *************/
...
/**** Error checking is bypassed here because Power flow user_analysis does not terminate
      the returned record with "return status ...." ****/

  sprintf(cmd,"/reports,select user_analysis,file=%s,output=%s",infile, outfile);
  printf("  %s\n", cmd);
  error = pf_command(cmd);

/**** Error checking is bypassed here because Power flow user_analysis does not terminate
      the returned record with "return status ...." ****/
  if (error!=0) { 
     cf_logErr("Unverified User analysis action - assumed OK:  Input file = %s\n", infile);
/*    *stat |= FAIL_USAN; */
/*    return;             */
  }

/******** this stuff sorta works but puts each file on a separate page *****
...
**************** Eliminate Control File Option ****************/
    ...
    if (strstr(sp, "PERTURBATION_FILE")!=NULL) {
        trace->query &= ~(QUERY_PRTRB);
        if (tp==NULL) return;
        strcpy(trace->prtrbSpec, tp);
        continue;
    }

    if (strstr(sp, "LIMIT_SOL_STEPS")!=NULL) {
        trace->query &= ~(QUERY_STPS);
        if (tp==NULL) return;
        sscanf(tp, "%d", &trace->lmt_sol);
        continue;
    }
    ...
    if (strstr(sp, "PERTURB")!=NULL) {
        ...
        if (strstr(tp, "FILE")!=NULL) trace->perturb = PERTURB_F;
        trace->query &= ~(QUERY_PERT);
        continue;
    }
    if (strstr(sp, "FILE_SAVE_FLAG")!=NULL) {
        trace->query &= ~(QUERY_FSF);
        if (tp==NULL) return;
        strcpy(trace->FilSavFlg, tp);
        continue;
    }
  }
  return;
...
void de_load_oldbase(int *stat, char *file)
{
  ...
  printf("\n  Loading Base... %s\n", file);
  sprintf(cmd,"/initalize");
/*  printf("  %s\n", cmd); */
  pf_command(cmd);
/************** temporary stuff to fix GEN_DROP retention peoblwm **********/
  sprintf(cmd,"/GEN_DROP");
/*  printf("  %s\n", cmd); */
  pf_command(cmd);
/************** temporary stuff to fix GEN_DROP retention peoblwm **********/
...
/******************** permit commands in common mode outage  *********/
/*    if ( s[0] == '>' ) { */
...
void   de_apply_perturbation(char *PrtrbId, FILE *prtrbFile)
{
  ...
  printf("  Seeking Perturbation... %s\n", PrtrbId);
  mode = 0;                                      /* searching */
  rewind(prtrbFile);
  strcpy(cmd, "/changes, file=*\n"); 
  while (fgets(s, MAX_IN, prtrbFile)!=NULL) {
    cf_str2upper(s);
    if ( s[0] == '.' ) continue;                 /* skip comments */

        if ( strstr(s, "> PERTURBATION") != NULL) {
    ...
    if ( mode == 1 ) {
        if ( strstr(s, "PERTURBATION_FILE")!=NULL ) {
            mode = 0;
        }
        else if ( strstr(s, "PERTURBATION ")!=NULL ) {
            mode = 1;
        }
        else {                                   /* unrecognized command */
            mode = 0;                            /* searching */
...
void de_pv_curve_pre(Trace *trace, Step *step)
{
  FILE   *dv0, *dv1, *kv0, *kv1, *mw0, *mw1, *qq0, *qq1;
  float   Xstart, Xstop, dX, X, Y, x, y, Ppct, Qpct, Xout, Xtotal;
  char    PrtrbId[80];
  char    title[MAX_IN];
  pf_rec *pbusRec;
  Link   *pbusLink, *pbusList, *list, *curBase, *TmpBaseList;
  Link   *curPrtrb; 
  int     setup;
  cf_Gen *pbusGen;
  pf_case_stats case_stats;
  int stat_error;
  int cnt_sol;
  char FilNam1[80] = "Pre";
  float Ppcts, Qpcts, Pfact, Qfact;
  cf_StrStr *PreFile_Id;
/************** debug stuff - add more info to execution file **********/
  char cmd[MAX_CFLOW_BUF];

  printf("  Making Pre-Outage PV curve...\n");
...
/***** new stuff for restarting from temp *.bse ****************/
...
/********** For each outage, initalize pbusGen->set = pbusGen->start ****/ 
/************ not needed (i think) because we only make one set of pre cases now
  pbusList = trace->pbusList;
  while (pbusList!=NULL) 
    {pbusLink = (Link *) pbusList->data;
     pbusGen = (cf_Gen *) ((Link *) pbusLink->next)->data;
     pbusGen->set = pbusGen->start;
...
/********** For each outage, initalize pbusGen->set = pbusGen->start ****/ 
  ...
  cnt_sol = 0;
  while (pbusList!=NULL || curPrtrb!=NULL) {
    cnt_sol = cnt_sol + 1;
/***    pbusLink = (Link *) pbusList->data; ***** moved this stuff *****/
/************************ heading logic **********************/
...
*********/
        ...
        ff_printDvdqHeader(dv0, trace->mbusList);
        ff_printVoltHeader(kv0, trace->mbusList);
        ff_printLoadHeader(mw0, trace->cutpList);
        ff_printQresHeader(qq0, trace->gbusList); /* qres, qgen */
...
/************************ heading logic **********************/
    ...
    if (trace->perturb == PERTURB_B) {
        ...
        pbusGen->set = X;
        de_set_gen(&trace->stat, trace->pbusList, trace->Pstart, trace->Pstop, Xstart);
    ...
    if (trace->perturb == PERTURB_L) {
        ...
        if (setup==0) {
            ...
            dX     =  1;
            X = Xstart;
/****** logic for restarting from temp *.bse ****************/
            Pfact = trace->Ppct / 100.0;
            Qfact = trace->Qpct / 100.0;
/****** logic for restarting from temp *.bse ****************/
        }
        Ppct = X * trace->Ppct;
        Qpct = X * trace->Qpct;
...
/****** logic for restarting from temp *.bse ****************/
        if (X == 0)
          ...
          {Ppcts = (((1 + X*Pfact) / (1 + (X-1)*Pfact)) - 1) * 100.0; 
           Qpcts = (((1 + X*Pfact) / (1 + (X-1)*Qfact)) - 1) * 100.0;
          }
/****** logic for restarting from temp *.bse ****************/
    }

    if (trace->perturb == PERTURB_F) 
       {
        strcpy( PrtrbId, curPrtrb->data);
        de_apply_perturbation (PrtrbId, trace->prtrbFile);
       }
    setup = 1;                                                   

/********************* save this /initdef command  ************/
    pf_user_init_def();                     
/********************* save this /initdef command  ************/
...
/********************* save this /initdef command stuff ************/
        printf("  X = %0.0f, Xstop = %0.0f, dP = %0.2f%%, %8.5f%%, dQ = %0.2f%% %8.5f%%, \n", 
            X, Xstop, Ppct, Ppcts, Qpct, Qpcts);                                         
        de_ch_par_load(&trace->stat, pbusRec, "V", &Y, "P", Ppcts, "Q", Qpcts, 
/********************* save this /initdef command stuff ************/
            trace->Pzones, trace->Powners, trace->Pareas, &x, &y);
        Xout = Ppct;
/******** define Xtotal  for "PERTURB_L" **********/
        Xtotal = 0.0;
/******** define Xtotal  for "PERTURB_L" **********/
    }
    if (trace->stat & FAIL_CRIT) 
      ...
      {           /**********solution NOT successful *******/
       cf_logBAD("Pre Outage Case failed at solution %d.  See file %s\n %s\n\n",
       cnt_sol,  step->preDV, step->title2);
       break;
      }
...
/*************** pre outage sol stuff *******************/       
    ...
    if (trace->stat & FAIL_CRIT) 
      {           /**********solution NOT successful *******/
       cf_logBAD("Pre Outage Case failed at solution %d.  See file %s\n %s\n\n",
       cnt_sol + 1,  step->pstDV, step->title2);
       break;
      }
...
/***** logic for restarting from temp *.bse ****************/
  sprintf(FilNam1,"PRE%s", trace->outSeri);
  if (trace->perturb == PERTURB_F)
    {
     strcat(FilNam1, PrtrbId);
     blank_2_undrbr(FilNam1);
     strcat(FilNam1, ".bse");
    }
  else
    {rand_file(FilNam1);}
  strcpy (trace->FilNam1, FilNam1);
  printf ("File1 in main = %s\n", trace->FilNam1);
  strcpy(step->oldBase, (char *) FilNam1);


/*  list = ff_FileFlt22Link (FilNam1, Xout, Xtotal); */

  if (trace->perturb == PERTURB_L)
   {sprintf(PrtrbId, "%6.3f|%6.0f|", Xout, Xtotal);}

  if (trace->perturb == PERTURB_B)
   {sprintf(PrtrbId, "%6.0f|%6.0f|", Xout, Xtotal);}

  if (trace->perturb == PERTURB_F) 
    {strcat(PrtrbId, "||");}

  list = ff_StrStrToLink (FilNam1, PrtrbId);

  cf_appList(&trace->baseListTmp, list); /* save temp base cases for use with outages */
  de_save_newbase(&trace->stat, step->oldBase);
/***** logic for restarting from temp *.bse ****************/
...
/*
    ff_printLoadPoints(mw0, trace->perturb, trace->stat, trace->cutpList, Xout, Xtotal);
    ff_printDvdqPoints(dv0, trace->perturb, trace->stat, trace->mbusList, Xout, Xtotal);
    ff_printVoltPoints(kv0, trace->perturb, trace->stat, trace->mbusList, Xout, Xtotal);
    ff_printQresPoints(qq0, trace->perturb, trace->stat, trace->gbusList, Xout, Xtotal);
*/
    ff_printLoadPoints(mw0, trace->perturb, trace->stat, trace->cutpList, PrtrbId);
    ff_printDvdqPoints(dv0, trace->perturb, trace->stat, trace->mbusList, PrtrbId);
    ff_printVoltPoints(kv0, trace->perturb, trace->stat, trace->mbusList, PrtrbId);
    ff_printQresPoints(qq0, trace->perturb, trace->stat, trace->gbusList, PrtrbId);
...
/*********** Good debug STUFF *************/
   ...
   while (TmpBaseList != NULL)
     {
      PreFile_Id = (cf_StrStr *) TmpBaseList->data;
      printf("Stored base case is: %s\n", PreFile_Id->str1);
      TmpBaseList = TmpBaseList->next;
     }
/*********** Good debug STUFF *************/
  if (trace->perturb == PERTURB_B ||trace->perturb == PERTURB_L) 
    {
     X = X + dX;
...
/************** end of function 'de_pv_curve_pre' *************/

/************** begin function 'de_pv_curve' *************/
void de_pv_curve     (Trace *trace, Step *step)
{
  FILE   *dv0, *dv1, *kv0, *kv1, *mw0, *mw1, *qq0, *qq1;
  float   Xstart, Xstop, dX, X, Y, x, y, Ppct, Qpct, Xout, Xtotal;
...
/************** debug stuff - add more info to execution file **********/
  printf("  Making Post-Outage PV curve...\n");
...
/******** Begin loop through saved base cases *****************/
  setup = 0;
  pbusList = trace->pbusList;
  TmpBaseList = trace->baseListTmp; 
  data_chk.cnt_sol = 0; 
/*********** Begin Primary Loop thru saved base cases *************/
  while (TmpBaseList != NULL)
  {
      PreFile_Id = (cf_StrStr *) TmpBaseList->data;
      printf("Stored base case is: %s\n", PreFile_Id->str1);
      strcpy(step->oldBase, (char *) PreFile_Id->str1);

       strcpy(PrtrbId, (char *) PreFile_Id->str2);
...
*************/
        ...
        ff_printDvdqHeader(dv1, trace->mbusList);
        ff_printVoltHeader(kv1, trace->mbusList);
        ff_printLoadHeader(mw1, trace->cutpList);
        ff_printQresHeader(qq1, trace->gbusList);
...
**************** Eliminate Control File Option ****************/
    ...
    stat_error = pf_case_info(&case_stats);

    if (case_stats.case_soln_status == 1) 
      {           /********** solution successful  *****************/
       data_chk.cnt_sol = data_chk.cnt_sol + 1;
       strcpy (data_chk.FilNam1, step->oldBase);
       if (data_chk.cnt_sol >= data_chk.max_sol) break;
      }
    else
      {           /**********solution NOT successful *******/
       cf_logBAD("Case failed at solution %d.  Solution status = %d.  See file %s\n %s\n\n", 
       data_chk.cnt_sol + 1, case_stats.case_soln_status, step->pstDV, step->title2);

       if ((data_chk.max_sol - data_chk.cnt_sol) >= (trace->lmt_sol + 1)) 
         data_chk.max_sol = data_chk.cnt_sol + (trace->lmt_sol + 1);      
...
/*
    ff_printLoadPoints(mw1, trace->perturb, trace->stat, trace->cutpList, Xout, Xtotal);
    ff_printDvdqPoints(dv1, trace->perturb, trace->stat, trace->mbusList, Xout, Xtotal);
    ff_printVoltPoints(kv1, trace->perturb, trace->stat, trace->mbusList, Xout, Xtotal);
    ff_printQresPoints(qq1, trace->perturb, trace->stat, trace->gbusList, Xout, Xtotal);
*/

    ff_printLoadPoints(mw1, trace->perturb, trace->stat, trace->cutpList, PrtrbId);
    ff_printDvdqPoints(dv1, trace->perturb, trace->stat, trace->mbusList, PrtrbId);
    ff_printVoltPoints(kv1, trace->perturb, trace->stat, trace->mbusList, PrtrbId);
    ff_printQresPoints(qq1, trace->perturb, trace->stat, trace->gbusList, PrtrbId);


    setup = 0;        

    TmpBaseList = TmpBaseList->next;
 }  /*********** End Primary Loop thru saved base cases *************/
...
/*********** multiple buffer logic **********/
  char bufaux[MAX_CFLOW_BUF];
  char *more_buf;
/*********** multiple buffer logic **********/
...
/* set up definitions */
  ...
  pf_user_sub_def("BASE");  /* returns data in the "reply_pf" buffer */

/*********** Begin multiple buffer logic **********/
  more_buf = strstr(reply_pf, "*[EOM]"); 
  if (more_buf != NULL)
  {
    strcpy (bufaux, "");
    do 
      { 
       more_buf = strstr(reply_pf, "*[EOM]"); 
       if (more_buf != NULL) *more_buf = '\0';
       scrunch_blanks(reply_pf, bufaux, MAX_CFLOW_BUF);  
       if (more_buf != NULL)  pf_user_sub_def("CONTINUE");  /* GET next BUFFER FULL */
      } while (more_buf != NULL);
    strcpy(reply_pf, bufaux);
  }
/*********** End multiple buffer logic **********/
...
/* read definitions */
  ...
  for (curLink=mbusList; curLink!=NULL; curLink=curLink->next) {
    ...
    cf_bus2rec((cf_Bus *) curData->data, &b);
    sprintf(symbol, "MDVQ%d", n);
    pf_user_quantity(symbol, ".DVQ", &dvdq);   /* searches reply_pf buffer  */
    sprintf(symbol, "MVK%d", n);
...
/***************** TESTING STUFF ****************
  if (perturb & PERTURB_L)
    {fprintf(fp, "%6.3f|%6.1f|", P, T);}
  else
    {fprintf(fp, "%6.1f|%6.1f|", P, T); }
***************** TESTING STUFF ****************/
/***************** TESTING STUFF ****************/
    {fprintf(fp, "%s", PrtrbId); }
/***************** TESTING STUFF ****************/
...
/*  ff_printHeadrNamesPB(fp, HdLd[3], HdRp[3], mbusList, pbusLink);  */
  ff_printHeadrNamesPB(fp, HdLd[3], HdRp[3], mbusList);
  return;
}
...
void ff_printHeadrNamesPB(FILE *fp, char *l, char *r, Link *mbusList)
{
  Link *curLink, *curData;
  char  id[132];
  pf_rec b;
...
void ff_printLoadHeader(FILE *fp, Link *cutpList)
{
  static char *HdLd[] = { /* lead */
    " ",
    " Pgen |Ptotal|",
...
/*  ff_printLdHdrNames(fp, HdLd[3], HdRp[3], cutpList, pbusLink); */
  ff_printLdHdrNames(fp, HdLd[3], HdRp[3], cutpList);
  return;
}
...
/*  ff_printHeadrBuses(fp, HdLd[3], HdRp[3], gbusList, pbusLink); */
  ff_printHeadrBuses(fp, HdLd[3], HdRp[3], gbusList); 
  return;
}
void ff_printQresPoints(FILE *fp, int perturb, int stat, Link *gbusList, char *PrtrbId)
{
...
/****  saved pre outage basecases scheme ********/
Link *ff_FileFlt22Link(char *file, float flt1, float flt2)
{ 
  Link   *link;
  cf_StrFlt2 *sf2;

  if (cf_isblank(file)) return NULL;
  link = cf_newLink(sizeof(cf_StrFlt2));
  if (link==NULL || link->data==NULL) return NULL;
  link->kind = CF_KIND_SF2;
  sf2 = (cf_StrFlt2 *) link->data;
    strcpy(sf2->file, file);
    sf2->flt1 = flt1;
    sf2->flt2 = flt2;
  return link;
}
...
Link *ff_StrStrToLink(char *str1, char *str2)
{
  ...
  if (cf_isblank(str1)) return NULL;
  link = cf_newLink(sizeof(cf_StrStr));
  if (link==NULL || link->data==NULL) return NULL;
  link->kind = CF_KIND_STRSTR;
  str = (cf_StrStr *) link->data;
    strcpy(str->str1, str1);
    strcpy(str->str2, str2);
  return link;
}
...
void de_set_gen(int *stat, Link *pbusLink, float Pstart, float Pstop, float Xstart)
{
  ...
  while (pbusLink!=NULL) {
    ...
    if (fabs (pbusRec->i.ACbus.Pgen ) < .00001  && Xstart != 0.0){
       pbusRec->i.ACbus.Qsch_Qmax = 0.0;
       pbusRec->i.ACbus.Qmin = 0.0;
    }  
...
int rand_file(char *FilNam)
{
  ...
  printf("\n Welcome to the Random File Name Generator\n");
  tim = time(NULL);
  printf ("Time = %d\n", tim);
/*
  sprintf(FilTmp,"FILE%d", tim);
*/
  sprintf(FilTmp,"%d", tim); 
  strcat(FilNam, FilTmp);
...
void blank_2_undrbr(char *str)
/******************************************************************************\
* blank_2_undrbr: (s - string)
* convert all blanks in string to _.
* char *str   - pointer to string to be converted
* Developed by: DMStefonek  Updated: 10/1/98
\******************************************************************************/
{
  char *cp;
  for( cp = str ; *cp != '\0' ; cp++ ) {
...
void scrunch_blanks(char *buf_src, char *buf_cmp, int buf_size)
/* This routine scrunches extra blanks out of the source string "buf_src"        */
/* and builds a compressed buffer "buf_cmp" replacing blank strings with a       */
/* single blank character.  Each call to "scrunch_blanks()" appends              */
/* non-blank strings to the compressed buffer "buf_cmp".  Therefore,             */
/* "buf_cmp" must have a NULL wherever appending is intended to begin.           */
/* Each non-blank string is arbitrarly limited to 80 characters. See "stry[80]". */
/* The original "buf_src", which must be NULL terminated, is not changed.        */
/* "buf_size" is the size of the buffer "buf_cmp".  Strings from "buf_src"       */
/* which would not completely fit in "buf_cmp" are ignored.                      */
/* Leading blanks in the "buf_src" are all ignored.                              */
{
  int the_end;
  char stry[80];
  char *istrstr;
  ...
  max_buf_cmp = buf_cmp + buf_size - 2;
/*  printf("\n Here we are in scrunch_blanks\n"); */
  for (the_end = sscanf(buf_src, "%80s", stry); the_end > 0; the_end = sscanf(buf_src, "%s", stry))
    {

/****************   check for buffer "buf_cmp" overflow **********/
     istrstr = strchr(buf_cmp, '\0');
     if (max_buf_cmp <= (strlen(stry) + istrstr))
       {
       printf("Blank-Scrunched Buffer Overflow \n");
       break;
       }
     else
...
/**************** Scrunch blanks ************/
       ...
       strcat (buf_cmp, stry);
       strcat (buf_cmp, " ");
       istrstr =  strstr(buf_src, stry);
       buf_src =  strlen(stry) + istrstr;
       }
    }
}
