...
C      File: busovrpt.f
C
C      Purpose: Routine to obtain a filtered bus over-voltage
C               report.
C
...
C****************************************************************
C
        subroutine busovrpt (in_buf, out_buf, scrfil)

        character in_buf * (*), out_buf * (*)
        integer scrfil
c
c       This subroutine returns output formatted records of filtered
c       buses with voltages over a user-specified value.
c
c       Output parameter:
c
...
c       Set up the page header
        ...
        if (scrfilx. gt. 0) then
           outbuf = 'Bus Overvoltage Summary'
           call rpnlod
 ...
 120    format (t39,'KV   P.U.')
  ...
  130            format(5x, a3, 2x, a2, 2x, a8, f7.1, 2x, a2,
     1                  f8.2, f7.4)
